...
def padding(mlen):
    ...
    mdi = mlen & 0x3F  # get the message digest index - used for determining the position of the last byte 
    length = (mlen << 3).to_bytes(8, 'big')  # calculate the length of the original message
    padlength = 55 - mdi if mdi < 56 else 119 - mdi  # use mdi to calculate required padding length
